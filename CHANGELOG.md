...
### Hinzugefuegt
- **Welle A — Bloom-Anker und Bearbeitungszeit in allen 27 Kursbild-Kapiteln.** Kapitel 01–10 plus Kapitel 26 hatten bisher keinen expliziten Bloom-Tag im Lernziel-Block (Kap. 11–25 hatten ihn schon). Jetzt traegt jedes Kapitel mindestens das anspruchsvollste Outcome mit `(Bloom X — verb)` als sichtbare kognitive Erwartung. Zweitens: pro Kapitel ein Block `> **Geschaetzte Bearbeitungszeit:** ~XX min (Lesen YY min + Lab ZZ min + Uebung WW min)`, der Selbstregulation unterstuetzt. Capstones (Kap. 10, 26) tragen Stufen-bezogene Mehrstunden-Schaetzungen (Stufe 1/2/3) statt einer einzelnen Zahl. `course/00-kursuebersicht.md §Arbeitsweise` macht die Konvention explizit und ergaenzt eine Heuristik fuer Selbsteinschaetzung bei Drift.
- **Welle B — Praekonzept-Karten ("Bevor du anfaengst — was vermutest du?") auf Kap. 03, 06, 11, 16.** Vier zusaetzliche neuralgische Stellen, an denen die Lerner-Vorstellungen explizit elizitiert werden, bevor das neue Schema aufgebaut wird (Kap. 03: Objekt-Identitaet ueber Handle vs. Attribute, Kap. 06: `KeyStore`-Alias-Sichtbarkeit vs. Cert-Plumbing, Kap. 11: PSS-/ECDSA-Parameter-Achsen, Kap. 16: HMAC als bewusste Wahl, nicht Notloesung). Konzeptionell konsistent zum 0.20-/0.21-Pattern (Kap. 01, 04, 08, 13, 17, 20, 21, 25). Damit traegt jedes Kapitel mit einer prominenten Misconception die Conceptual-Change-Mechanik im Header.
- **Welle C — Bloom-Tags `(Recall)/(Analyse)/(Evaluate)/(Create)` in den Reflexionsfragen von Uebungen 00, 01, 07–21.** Uebungen 02–06 hatten das Pattern aus 0.20 schon; jetzt traegt jede Uebung mindestens drei Bloom-Stufen explizit in den Reflexionsfragen, gehoben auf einen Recall+Analyse+Evaluate-Mix (Uebung 21 zusaetzlich Create). Das macht die kognitive Tiefe der Reflexion fuer Lerner antizipierbar und liefert Uebungs-bezogene Self-Assessment.
- **Welle D — Worked-Example/Faded-Pattern auf Kap. 14, 15, 17, 24** — Cognitive-Load-Refactoring nach dem Muster, das in Kap. 13 (Hybrid-Verschluesselung) schon stand. Pro Kapitel ein vollstaendig durchgearbeiteter Pfad (CMS-Bash, Streaming-Bash mit pkcs11-spy-Beweis, Pooling-Go, ECDH-Go) plus drei Faded Examples mit je drei Leitfragen, die das Schema des Worked Example gegen die jeweilige Sprach-Implementierung pruefen. Reduziert die Element-Interactivity in den vier Vertiefungskapiteln, die parallele Sprach-Pfade praesentieren.
- **Welle E — Pfad-Navigation `> **Didaktischer Pfad:** Vorher → [...] · Nachher → [...]`** in jedem der 27 Kursbild-Kapitel. Die Datei-Nummerierung folgt thematischen Gruppen, der didaktisch empfohlene Pfad weicht ab (Kap. 12 nach Kap. 06, Kap. 08 nach Kap. 12, Kap. 11 vor Kap. 07/09 etc., siehe `course/00-kursuebersicht.md §Lernpfad`). Bisher gewann beim Repository-Browsing die Datei-Reihenfolge; jetzt fuehrt jeder Kapitel-Header explizit den didaktischen Vorgaenger und Nachfolger. Kosten: 27 zusaetzliche Header-Zeilen. Nutzen: ein Lerner, der direkt in Kap. 11 einsteigt, sieht sofort, dass davor Kap. 08 (Debugging) gehoert und danach Kap. 07 (Service-Integration) kommt — nicht Kap. 12 wie die Datei-Reihenfolge suggerieren wuerde.

### Didaktischer Hintergrund
- Diese Iteration setzt das eigene Folge-Review zur 0.21.0 um (Bezugsrahmen unveraendert: Biggs' konstruktives Alignment, Anderson/Krathwohl-Taxonomie, Cognitive Load Theory, Modell der Didaktischen Rekonstruktion). Das Review fand keine inhaltlichen Luecken, sondern eine **Konsistenz-Luecke**: die in 0.19/0.20/0.21 etablierten Patterns (Bloom-Tags, Praekonzept-Karten, Worked-Example, gestufte Reflexionsfragen) waren ungleich verteilt. 0.22 zieht sie systematisch ueber alle Kapitel und Uebungen.
- Konkret: (Welle A) Bloom-Anker auf allen Kapiteln statt nur Vertiefungsmodulen, Bearbeitungszeit als Selbstregulations-Hilfe (Sweller: Lernregulation reduziert extraneous load); (Welle B) Conceptual-Change-Hooks auf vier weiteren neuralgischen Stellen (Posner et al.: kognitiver Konflikt als Vorbedingung fuer Akkommodation); (Welle C) Bloom-Coverage in Uebungen explizit, damit Reflexion antizipierbar wird (Biggs: Assessment-Sichtbarkeit als Teil von Alignment); (Welle D) Worked-Example-Effekt auf vier weitere Vertiefungskapitel (Sweller/Renkl: gefadete Beispiele reduzieren intrinsische Last beim Schema-Aufbau); (Welle E) didaktischer Pfad mit Vorgaenger/Nachfolger sichtbar, statt nur in der zentralen Uebersichts-Datei (Kattmann: didaktische Strukturierung als Lese-Affordanz).
- Nichts an der Substanz des Kurses wurde umgebaut; alle Aenderungen sind additiv oder Pattern-Vereinheitlichungen. Die Capstones (Kap. 10, 26) bleiben strukturell identisch zu 0.20/0.21.
